<template>
  <div class="min-h-screen bg-[#0f172a] text-slate-200 p-5 font-sans">
    <div class="relative py-8 text-center">
      <h1 class="text-4xl font-extrabold tracking-widest bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-600 animate-pulse">
        天機運勢
      </h1>
      <div class="mt-2 text-xs tracking-widest text-slate-500 uppercase">AI Destiny Insight</div>
    </div>

    <div v-if="data?.chart" class="mt-6 backdrop-blur-xl bg-white/5 border border-white/10 rounded-3xl p-6 shadow-2xl">
      <div class="grid grid-cols-4 gap-2 text-center">
        <div v-for="(word, i) in data.chart.eightWords.split(' ')" :key="i" class="flex flex-col">
          <span class="text-xs text-slate-400 mb-1">{{ ['年', '月', '日', '時'][i] }}</span>
          <div class="bg-gradient-to-b from-slate-700 to-slate-800 p-3 rounded-xl border border-white/5 shadow-inner">
    ...
    <div v-if="data?.analysis" class="mt-8 space-y-4">
      ...
      <div class="bg-slate-800/50 rounded-2xl p-5 leading-relaxed text-slate-300 border-l-4 border-purple-500 shadow-lg">
        {{ data.analysis }}
      </div>
    </div>
